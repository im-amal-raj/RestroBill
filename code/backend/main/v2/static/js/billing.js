...
function addRow(name, price, pid) {
    ...
    if (existingRow) {
        ...
        cart[newSiNo] = { "pid": pid, "name": name, "qty": quantity, "mrp": price, "price": (quantity * price).toFixed(2)};
        qtyListeners();
        reIndexSiNumbers();
    }
...
function handleQuantityChange(event) {
    ...
    if (!isNaN(quantity)) {
        ...
        updatePrice(row, siNo);
        updateTotal(calculateTotal());
    }
}

function updatePrice(row, siNo) {
    ...
    cart[siNo].mrp = mrp;
    cart[siNo].price = (quantity * mrp).toFixed(2);
}
...
document.getElementById("print").addEventListener("click", function () {
    //if (print_validate() ;
    cart["payment"] = { "paytype": payment.value,
